...
void main() 
{
	...
	vec4 tangentWS = inTangent;

	float sgn = inTangent.w;      // should be either +1 or -1
	vec3 bitangent = sgn * cross(normalWS.xyz, tangentWS.xyz);
	mat3 tangentToWorld = mat3(tangentWS.xyz, bitangent.xyz, normalWS.xyz);
